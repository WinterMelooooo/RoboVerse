...
from diffusion_policy.model.vision.multimodal_encoder import MultiModalEncoder
...
class MultiModalPolicyPro(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], use_gt_sensor = False) -> Dict[str, torch.Tensor]:
        """
        ...
        if self.obs_as_global_cond:
            ...
            nobs_features, pred_sensor = self.obs_encoder(this_nobs, use_gt_sensor)
    ...
    def compute_loss(self, batch, use_gt_sensor):
        # normalize input
        ...
        if self.obs_as_global_cond:
            ...
            if pnt_cloud_spUnet is not None:
                this_nobs["point_cloud"] = pnt_cloud_spUnet
            nobs_features, pred_sensor = self.obs_encoder(this_nobs, use_gt_sensor=use_gt_sensor)
            # reshape back to B, Do
        ...
        sensor_pred_loss = 0.0
        for sensor_name, sensor_pred in pred_sensor.items():
            sensor_gt = this_nobs[sensor_name]
            sensor_pred_loss += F.l1_loss(
                sensor_pred,
                sensor_gt,
                reduction="mean",
            )
