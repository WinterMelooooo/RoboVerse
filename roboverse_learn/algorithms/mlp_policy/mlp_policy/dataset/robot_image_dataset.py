...
import numpy as np
import torch
from mlp_policy.common.normalize_util import get_image_range_normalizer
...
class RobotImageDataset(BaseImageDataset):
    ...
    def postprocess(self, samples, device):
        ...
        assert agent_pos.shape[1] == 1, f"Expected agent_pos shape [B, {self.horizon}, D], got {agent_pos.shape}"
        assert head_cam.shape[1] == 1, f"Expected head_cam shape [B, {self.horizon}, 3, H, W], got {head_cam.shape}"
        assert action.shape[1] == 1, f"Expected action shape [B, {self.horizon}, D], got {action.shape}"
        return {
            "obs": {
