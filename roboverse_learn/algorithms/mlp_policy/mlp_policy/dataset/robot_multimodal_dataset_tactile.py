...
import numpy as np
import torch
from mlp_policy.common.normalize_util import get_identity_normalizer, get_image_range_normalizer
from mlp_policy.common.pytorch_util import dict_apply
from mlp_policy.common.replay_buffer import ReplayBuffer
from mlp_policy.common.sampler import (
    SequenceSampler,
    downsample_mask,
    get_val_mask,
)
...
from roboverse_learn.algorithms.mlp_policy.mlp_policy.dataset.robot_pointcloud_dataset import (
    ROBOT_ROOT_STATES,
    transform_point_cloud,
)
from roboverse_learn.algorithms.mlp_policy.mlp_policy.dataset.robot_spUnet_dataset import point_collate_fn
...
class MultiModalDatasetTactile(BaseImageDataset):
    def __init__(
        ...
        self.replay_buffer: ReplayBuffer = ReplayBuffer.copy_from_path(
            ...
            keys=["head_camera", "state", "action", "head_camera_pnt_cloud", "head_camera_depth", "sensors"],
        )
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        for sensor_name in self.sensor_names:
            norm_dict[sensor_name] = get_identity_normalizer
        fit_dict = {
    ...
    def postprocess(self, samples, device):
        ...
        data = self.add_sensors(samples, data)
    ...
    def add_pntcloud(self, samples, agent_pos, head_cam, action, device):
        ...
        if not self.norm_pnt_cloud:
            ...
            if not (len(point_cloud.shape) == 4 and point_cloud.shape[2] == 4096 and point_cloud.shape[3] == last_dim):
                raise ValueError(
                    f"point_cloud.shape = {point_cloud.shape}, while expecting to be (B, T, 4096, {last_dim})"
                )
    ...
    def add_depth(self, samples, head_cam, device):
        depth = (
            samples["head_camera_depth"][:, :, :1, :, :].to(device, non_blocking=True) / 255.0
        )  # (B, T, 4, H, W) [0,1]
        head_cam = torch.cat([head_cam, depth], dim=2)  # B, T, 4, H, W
        return head_cam

    def add_sensors(self, samples, data):
        for sensor_name in self.sensor_names:
            sensor_data = samples[f"sensors/{sensor_name}"]
            data["obs"][sensor_name] = sensor_data
        return data
