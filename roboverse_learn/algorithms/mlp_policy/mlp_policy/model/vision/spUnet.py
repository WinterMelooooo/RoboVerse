...
- https://github.com/Pointcept/Pointcept/blob/main/pointcept/models/sparse_unet/spconv_unet_v1m3_pdnorm.py
"""
...
class PDBatchNorm(torch.nn.Module):
    ...
    def forward(self, feat, condition=None, context=None):
        if self.decouple:
            ...
            _feat = 0
            for c, bn in zip(self.conditions, self.bns):
                apply = 1 if c == condition else 0
                _feat = _feat + bn(feat) * apply
            feat = _feat
...
class BasicBlock(spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        if self.in_channels == self.embed_channels:
            ...
            residual = residual.replace_feature(self.proj_norm(self.proj_conv(residual).features, condition, context))
...
class SpUNet(nn.Module):
    def __init__(
        ...
        self.num_channels = num_classes if num_classes > 0 else final_in_channels

        if pretrained_path is not None:
            self.load_ponderv2_weights(pretrained_path)
    ...
    def load_ponderv2_weights(self, path):
        from collections import OrderedDict
        ...
        for key, value in checkpoint["state_dict"].items():
            if key.startswith("module.backbone."):
                weight[key.replace("module.backbone.", "")] = value
            elif key.startswith("module.embedding_table"):
    ...
    def forward(self, input_dict):
        ...
        condition = input_dict["condition"][0] if "condition" in input_dict.keys() else self.conditions[0]
        context = (
            input_dict["context"]
            if "context" in input_dict.keys()
            else (
                self.embedding_table(torch.tensor([self.conditions.index(condition)], device=grid_coord.device))
...
class SpUnetEncoder(ModuleAttrMixin):
    def __init__(
        self,
        shape,
        pcd_model: SpUNet,
    ...
    def forward(self, pcd):
        if not isinstance(pcd, Dict):
            ...
            return torch.zeros((batch_size, self.projector_channels[-1]), dtype=self.dtype, device=self.device)
        batch_size = None
