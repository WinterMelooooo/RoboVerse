...
class MultiModalEncoderFine(ModuleAttrMixin):
    def __init__(
        self,
        shape_meta: dict,
        multimodal_encoder,
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == "rgb" or type == "rgbd":
                img_keys.append(key)
                key_transform_map[key] = self._impl_img_transform(shape, type, key, img_transform_args)
            elif type == "low_dim":
    ...
    def forward(self, obs_dict):
        ...
        for key in self.point_cloud_keys:
            ...
            new_obs["pntcloud"] = pnt_cloud

        # concatenate all features
        result = self.multimodal_encoder(**new_obs)
        return result
