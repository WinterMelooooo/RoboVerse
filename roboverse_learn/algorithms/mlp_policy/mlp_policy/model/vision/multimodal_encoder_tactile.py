...
import torchvision
from mlp_policy.common.pytorch_util import dict_apply, replace_submodules
from mlp_policy.model.common.module_attr_mixin import ModuleAttrMixin
from mlp_policy.model.vision.crop_randomizer import CropRandomizer
...
class MultiModalEncoderTactile(ModuleAttrMixin):
    def __init__(
            ...
            if type == "rgb" or type == "rgbd":
                ...
                if this_model is not None:
                    ...
                    if img_encoder_args.get("pretrained", None):
                        ...
                        sub_sd = {k[len(prefix) + 1 :]: v for k, v in full_sd.items() if k.startswith(prefix + ".")}
        ...
        self.dropout_model = (
            ...
            if not fusion_args.get("use_channel_wise_dropout", False)
            else ChannelWiseDropout(p=self.dropout)
        )
        self.prediction_model = prediction_model
        cprint(
            f"[MultiModal Encoder]: channel wise dropout: {fusion_args.get('use_channel_wise_dropout', False)}", "cyan"
        )
        try:
    ...
    def forward(self, obs_dict, use_gt_sensor):
        ...
        for key in self.img_keys:
            ...
            assert img.shape[1:] == self.key_shape_map[key], f"{img.shape} vs {self.key_shape_map[key]}"
        ...
        for key in self.point_cloud_keys:
            ...
            if batch_size_pnt is None:
                ...
                if isinstance(pnt_cloud, Dict):
                    first_key = next(iter(pnt_cloud))
                    assert batch_size_pnt == len(pnt_cloud[first_key]), (
                        f"batch_size_pnt mismatch for key {key}: {batch_size_pnt} vs {len(pnt_cloud[first_key])}"
                    )
                else:
        ...
        fused = self.primary_fusion_func(
            img_features=img_features,
            low_dim_features=low_dim_features,
            pntcloud_features=pntcloud_features,
            pres_sensor_state_features=pres_sensor_state_features,
        )
        ...
        for sensor_name, predict_sensor_state in pred_sensors_state.items():
            if predict_sensor_state.shape[0] != batch_size:
                raise ValueError(f"Batch size mismatch: {predict_sensor_state.shape} vs {batch_size}")
    ...
    def _impl_img_transform(self, shape, type, key, args):
        ...
        if args.imagenet_norm:
            ...
            print(f"{key} mean: {[2 * x - 1 for x in mean]}, std: {[2 * x for x in std]}")
    ...
    def _get_attention_func(
        ...
        self.n_tokens = (
            len(self.img_keys) + len(self.point_cloud_keys) + len(self.low_dim_keys) + len(self.sensor_state_keys)
        )
        ...
        if use_modality_encoding:
            self.modality_embed = nn.Embedding(
                self.n_tokens, embed_dim
            )  # 3+n+1 modalities: img, pc, state, n sensor(present + predict), fused
    ...
    def _primary_cross_attention_features(
        self, img_features, low_dim_features, pntcloud_features, pres_sensor_state_features
    ):
        ...
        if self.use_modality_encoding:
            ...
            start_img, start_pc, start_state, start_sensor = (
                0,
                n_feats[0],
                n_feats[0] + n_feats[1],
                n_feats[0] + n_feats[1] + n_feats[2],
            )
            # 0, 1, 2, 3
    ...
    def _secondary_cross_attention_features(self, fused_features, pred_sensor_state_features):
        ...
        if self.use_modality_encoding:
            ...
            pred_sensor_state_features = [
                f + embed(idx + sensor_start_idx) for idx, f in enumerate(pred_sensor_state_features)
            ]
