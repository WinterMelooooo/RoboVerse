...
class MultiImageObsEncoder(ModuleAttrMixin):
    def __init__(
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == "rgb":
                ...
                if imagenet_norm:
                    ...
                    if not test_rescale:
                        this_normalizer = torchvision.transforms.Normalize(mean=mean, std=std)
                ...
                if imagenet_norm:
                    ...
                    if not test_rescale:
                        ...
                        print(f"{task_name} mean: {[2 * x - 1 for x in mean]}, std: {[2 * x for x in std]}")
        ...
        print(f"[INFO]: Using img:{use_img}")
        ...
        if self.use_img:
            ...
            if self.share_rgb_model:
                ...
                for key in self.rgb_keys:
                    img = obs_dict[key]
                    if key == "point_cloud":
                        img = img[:, :, :3].permute(0, 2, 1) if img.shape[2] == 6 or img.shape[2] == 3 else img
                    if batch_size is None:
                        batch_size = img.shape[0]
                    else:
                        assert batch_size == img.shape[0]
