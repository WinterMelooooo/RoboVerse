...
class MultiModalEncoder(ModuleAttrMixin):
    ...
    def _get_attention_func(
        ...
        if post_fusion_func == "sum":
            ...
            self.post_fusion_mlp = nn.Sequential(
                nn.Linear(2 * embed_dim, embed_dim), nn.ReLU(), nn.Linear(embed_dim, embed_dim)
            )
    ...
    def _get_algin_and_fusion_func(
        self, fusion_func, fusion_params, post_fusion_func, post_fusion_params, final_proj_func, final_proj_params
    ):
        if fusion_func == "cat":
        ...
        if final_proj_func == "linear":
            ...
            if final_proj_params.get("norm", False):
                self.final_proj = nn.Sequential(self.final_proj, nn.LayerNorm(final_proj_params["out_dim"]))
