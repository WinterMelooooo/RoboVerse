...
from roboverse_learn.algorithms.mlp_policy.mlp_policy.model.vision.dp3_encoder import PointNetLayerEncoderXYZ
...
class FusionLateEncoder(nn.Module):
    def __init__(self, rgb_params, pntcloud_params, action_params, fusion_params, add_uv=False):
        ...
        self.last_block_channel = (
            3 * self.rgb_params.rgb_hidden_dim + self.pointnet.block_channel[-1] + self.fusion_params.uv_hidden_dim
        )
    ...
    def forward(self, rgb, pntcloud, action):
        ...
        fused = self.fusion_func(img_features, pointcloud_features, uv_features)
    ...
    def _sample_features_from_uv(
        self, feat_map: torch.Tensor, uv: torch.Tensor, img_shape: list, sample_mode: str
    ) -> torch.Tensor:
        """
        Args:
        ...
        if sample_mode == "bilinear":
            ...
            sampled = F.grid_sample(
                feat_map, grid, mode=sample_mode, padding_mode="border", align_corners=True
            )  # [B, C, N, 1]
            ...
            feat_perm = feat_map.permute(
                0, 2, 3, 1
            ).contiguous()  # (B, C_img, H_down, W_down) -> (B, H_down, W_down, C_img)
            B, N = uv.shape[:2]  # (B, N)
...
class FusionEarlyEncoder(nn.Module):
    def __init__(self, rgb_params, pntcloud_params, action_params, fusion_params, add_uv=False):
        ...
        self.resnet.avgpool = nn.Identity()
        self.resnet.fc = nn.Identity()
        self.resnet.mlp_layer2 = (
            nn.Sequential(
                nn.Linear(self.resnet_out_channels[1], self.rgb_params.rgb_hidden_dim),
                nn.LayerNorm(self.rgb_params.rgb_hidden_dim),
                nn.ReLU(),
            )
            if self.rgb_params.rgb_hidden_dim is not None
            else nn.Identity()
        )
        self.resnet.mlp_layer3 = (
            nn.Sequential(
                nn.Linear(self.resnet_out_channels[2], self.rgb_params.rgb_hidden_dim),
                nn.LayerNorm(self.rgb_params.rgb_hidden_dim),
                nn.ReLU(),
        ...
        self.layer1_in_channels = (
            (self.resnet_out_channels[1] if not self.rgb_params.rgb_hidden_dim else self.rgb_params.rgb_hidden_dim)
            + self.pointnet.block_channel[0]
            + (self.fusion_params.uv_hidden_dim if self.add_uv else 0)
        )
        self.mlp_layer1 = FusionMLP(
            self.layer1_in_channels, mid_channels[0], self.pointnet.block_channel[0], use_layernorm=True
        )  # -> [B, N, 64]
        self.layer2_in_channels = (
            (self.resnet_out_channels[2] if not self.rgb_params.rgb_hidden_dim else self.rgb_params.rgb_hidden_dim)
            + self.pointnet.block_channel[1]
            + (self.fusion_params.uv_hidden_dim if self.add_uv else 0)
        )
        self.mlp_layer2 = FusionMLP(
            self.layer2_in_channels, mid_channels[1], self.pointnet.block_channel[1], use_layernorm=True
        )  # -> [B, N, 128]
        self.layer3_in_channels = (
            (self.resnet_out_channels[3] if not self.rgb_params.rgb_hidden_dim else self.rgb_params.rgb_hidden_dim)
            + self.pointnet.block_channel[2]
            + (self.fusion_params.uv_hidden_dim if self.add_uv else 0)
        )
        self.mlp_layer3 = FusionMLP(
            self.layer3_in_channels, mid_channels[2], self.pointnet.block_channel[2], use_layernorm=True
        )  # -> [B, N, 256]
    ...
    def forward(self, rgb, pntcloud, action):
        ...
        rgb_feat2 = self._sample_features_from_uv(
            rgb_feat_map2, uv, self.rgb_shape, self.rgb_sample_mode
        )  # [B, N, 128]
        rgb_feat2 = self.resnet.mlp_layer2(rgb_feat2)  # [B, N, rgb_hidden_dim]
        uv_feat = None
        if self.add_uv:
            uv_feat = self.uv_mlp(uv)
        aggre_feat1 = self.fusion_func(rgb_feat2, pnt_feat1, uv_feat)  # [B, N, rgb_hidden_dim + 64 + uv_hidden_dim]
        ...
        rgb_feat3 = self._sample_features_from_uv(
            rgb_feat_map3, uv, self.rgb_shape, self.rgb_sample_mode
        )  # [B, N, 256]
        rgb_feat3 = self.resnet.mlp_layer3(rgb_feat3)  # [B, N, rgb_hidden_dim]
        aggre_feat2 = self.fusion_func(rgb_feat3, pnt_feat2, uv_feat)  # [B, N, rgb_hidden_dim + 128 + uv_hidden_dim]
        ...
        rgb_feat4 = self._sample_features_from_uv(
            rgb_feat_map4, uv, self.rgb_shape, self.rgb_sample_mode
        )  # [B, N, 512]
        rgb_feat4 = self.resnet.mlp_layer4(rgb_feat4)  # [B, N, rgb_hidden_dim]
        aggre_feat3 = self.fusion_func(rgb_feat4, pnt_feat3, uv_feat)  # [B, N, rgb_hidden_dim + 256 + uv_hidden_dim]
