...
def get_composite_scheduler(
    ...
    if fill:
        # Fill the rest of the lr_lambda with the default scheduler
        cprint(f"Filling {len(optimizer.param_groups) - len(lr_lambda)} additional lr_lambda with the default scheduler: {default_scheduler_name}", "yellow")
        while len(lr_lambda) < len(optimizer.param_groups):
            lr_lambda.append(lr_lambda[0])
