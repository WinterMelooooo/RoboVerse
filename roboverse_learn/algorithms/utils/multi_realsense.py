        ...
        if self.enable_depth:
            ...
            if self.enable_pointcloud:
                ...
                point_cloud_frame = self.create_colored_point_cloud(
                    color_frame, depth_frame, far=self.z_far, near=self.z_near, num_points=self.num_points
                )
            else:
...
class MultiRealSense(object):
    def __init__(
        ...
        if use_front_cam:
            ...
            self.front_extrinsics = torch.tensor(
                [
                    [0.0, 1.0, 0.0, 0.0],
                    [0.7071067094802856, 0.0, -0.70710688829422, 0.0],
                    [-0.7071069478988647, 0.0, -0.7071067094802856, 2.1213204860687256],
                    [0.0, 0.0, 0.0, 1.0],
                ],
                device="cuda",
            )
            cprint(
                f"[Multi Realsense]Front extr hard coded, assuming cam pos=(1.5, 0, 1.5), cam_look_at=(0, 0, 0)",
                "red",
            )
        if use_right_cam:
    ...
    def __call__(self):
        ...
        if self.use_front_cam:
            ...
            cam_dict.update({
                "head_camera": front_color,
                "head_camera_depth": front_depth,
                "head_camera_pnt_cloud": front_point_cloud,
            })
    ...
    def front_intr(self):
        if self.use_front_cam:
            cam_info = self.front_process.camera_info
            return torch.tensor(
                [
            ...
            raise ValueError("Front camera is not enabled.")

    def front_extr(self):
        if self.use_front_cam:
            return self.front_extrinsics
        else:
            raise ValueError("Front camera is not enabled.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # imageio.imwrite(f"color_front.png", out["head_camera"])
        ...
        # visualizer.visualize_pointcloud(out["head_camera_pnt_cloud"])
        # cam.finalize()
        pipeline = cam.front_process.pipeline
        align = cam.front_process.align
        ...
        img = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR) if len(rgb.shape) == 3 else rgb.copy()
        # 画一个半径为5的红色实心圆，线宽 -1 表示填充
